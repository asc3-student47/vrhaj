...
- **WHEN** a request includes a SKU present in one or more supplier contracts
...
- **WHEN** a matching contract expiration date is on or before the evaluation date
...
- **WHEN** a matching contract available quantity is less than requested quantity
...
- **WHEN** a matching contract lead time is greater than the requested lead time
...
- **WHEN** multiple eligible supplier contracts are returned for a request
...
- **WHEN** supplier evaluation completes
- **THEN** each evaluated contract result includes eligibility status and machine-readable reasons for inclusion or exclusion
